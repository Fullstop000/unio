...
func TestListSessionsReadsClaudeHistory(t *testing.T) {
	...
	got, err := New(context.Background(), driver.AgentSpec{}).ListSessions(driver.ListSessionsParams{Cwd: "/repo/api"})
	if err != nil {
		t.Fatal(err)
	}
	...
	all, err := New(context.Background(), driver.AgentSpec{}).ListSessions(driver.ListSessionsParams{})
